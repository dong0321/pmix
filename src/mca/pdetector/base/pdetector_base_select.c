...
int pmix_pdetector_base_select(void)
{
    pmix_mca_base_component_list_item_t *cli = NULL;
    pmix_pdetector_base_component_t *component = NULL;
    pmix_pdetector_active_module_t *newactive, *active;
    ...
    PMIX_LIST_FOREACH(cli, &pmix_pdetector_base_framework.framework_components, pmix_mca_base_component_list_item_t) {
        component = (pmix_pdetector_base_component_t *) cli->cli_component;

        pmix_output_verbose(5, pmix_pdetector_base_framework.framework_output,
                            "mca:pdetector:select: checking available component %s",
        ...
        newactive = PMIX_NEW(pmix_pdetector_active_module_t);
        newactive->priority = pri;
        newactive->component = component;
        newactive->module = (pmix_pdetector_base_module_t*)mod;
        ...
        PMIX_LIST_FOREACH(active, &pmix_pdetector_base.actives, pmix_pdetector_active_module_t) {
            if (newactive->priority > active->priority) {
                pmix_list_insert_pos(&pmix_pdetector_base.actives,
        ...
        if (!inserted) {
            ...
            pmix_list_append(&pmix_pdetector_base.actives, &newactive->super);
        }
    }
